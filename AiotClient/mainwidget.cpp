...
{
    ui->setupUi(this);
    pTab1DevControl = new Tab1DevControl(ui->pTab1);//객체 생성
    ui->pTab1->setLayout(pTab1DevControl->layout());//비율대로 크기 설정 하기
    ...
    ui->pTabWidget->setCurrentIndex(4);//tab1을 디폴트 화면으로 보여줌
    ...
    //connect(pTab3ControlPannel, SIGNAL(LampSig(bool)),pTab2SocketClient,SLOT(socketLamp(bool)));
    //connect(pTab3ControlPannel, SIGNAL(PlugSig(bool)),pTab2SocketClient,SLOT(socketPlug(bool)));
