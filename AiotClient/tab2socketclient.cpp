    ...
    , ui(new Ui::Tab2SocketClient)
{
    ...
    ui->pPBSend->setEnabled(false);//처음엔 버튼 비활성화
...
void Tab2SocketClient::on_pPBserverConnect_toggled(bool checked)
{
    ...
    if(checked)
    {
        ...
        if(bFlag)
        {
            ...
            ui->pPBSend->setEnabled(true); //버튼 활성화
        }
    }
    else
    {
...
void Tab2SocketClient::updateRecvDataSlot(QString strRecvData)
{
    ...
    //[MJ_QT]LED@0xff ==> @MJ_QT@LED@0xff
    strRecvData.replace("[", "@"); //@로 바꿈
    strRecvData.replace("]", "@");
...
void Tab2SocketClient::on_pPBSend_clicked()
{
    ...
    ui->pLEsendData->clear();   // line edit 메시지 보낸 후 적힌 문자 clear
}

void Tab2SocketClient::socketSendToLinux(int keyNo)
{
    pSocketClient->socketWriteDataSlot("[MJ_LIN]KEY@"+QString::number(keyNo));
}
/*
void Tab2SocketClient::socketLamp(bool clicked)
{
    if(clicked)
        pSocketClient->socketWriteDataSlot("[MJ_LIN]LALMP@ON");
    else
        pSocketClient->socketWriteDataSlot("[MJ_LIN]LALMP@OFF");

}

void Tab2SocketClient::socketPlug(bool clicked)
{
    if(clicked)
        pSocketClient->socketWriteDataSlot("[MJ_LIN]PLUG@ON");
    else
        pSocketClient->socketWriteDataSlot("[MJ_LIN]PLUG@OFF");
